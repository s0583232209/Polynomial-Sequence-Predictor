"""
Train an LSTM model to predict the next values of polynomial sequences.

This script:
    1. Loads the training, validation, and test datasets.
    2. Creates and trains the LSTM model.
    3. Uses AdamW optimization with learning-rate scheduling.
    4. Applies gradient clipping for stable training.
    5. Uses early stopping to prevent overfitting.
    6. Saves the best-performing model.
    7. Evaluates the final model on the test set.
...
# AdamW optimizer combines adaptive learning rates with
...
# Reduce the learning rate whenever validation loss
# stops improving.
...
# Display one batch to verify that normalization
...
def evaluate(model, loader, mse_criterion, mae_criterion, device):
    ...
    Args:
        ...
        loader: DataLoader containing validation or test data.
        mse_criterion: Mean Squared Error loss function.
        mae_criterion: Mean Absolute Error loss function.
        device: CPU or CUDA device.

    Returns:
        tuple:
            (average_mse, average_mae)
    """
...
for epoch in range(NUM_EPOCHS):
    ...
    for x, y in train_loader:
        ...
        # Clip gradients to stabilize LSTM training.
